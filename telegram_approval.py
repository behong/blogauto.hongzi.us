...
def send_publication_approval(summary: list[dict[str, Any]], source: str = "toss-daily") -> dict[str, Any]:
    """Create a short-lived approval batch and send one Telegram inline keyboard."""
    ...
    lines = [
        ...
        *[_brief_item(item, index) for index, item in enumerate(summary, start=1)],
        ...
        f"승인 유효 시간: {APPROVAL_TTL_MINUTES}분",
        "승인 후에도 실제 공개 전송 전 최종 재검증을 수행합니다.",
    ]
    keyboard = {
        "inline_keyboard": [
            [
